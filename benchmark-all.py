...
def run_benchmarks_with_scheduler(SCHEDULER, FLAGS):
  ...
  console = Console()
  if SCHEDULER:
    scheduler_process = subprocess.Popen(["./run.sh", str(SCHEDULER)] + FLAGS, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
  else:
  ...
  for BENCHMARK in BENCHMARKS:
    ...
    with open('output.txt', 'w') as f, Live(console=console, refresh_per_second=4) as live:
          ...
          if fd == benchmark_process.stdout.fileno():
            ...
            if benchmark_line:
              ...
              if "completed (" in benchmark_line:
                ...
                ####################################
                # Save data (if final iteration)
                ####################################
                if f"iteration {ITERATIONS - 1}" in benchmark_line:
                  benchmark_name = re.search(r'====== (\S+) \(', benchmark_line).group(1)
      ...
      results["times"] = [sum(values) for values in zip(*[data["times"] for data in results.values()])]

      if not os.path.exists(f'{DATA_FOLDER}{experiment_name}'):
        os.makedirs(f'{DATA_FOLDER}{experiment_name}')
